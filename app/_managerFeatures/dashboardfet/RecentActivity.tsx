...
function RecentActivity() {
  return (
    <div className="w-full max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
      <h2 className="text-xl font-bold text-[#1a2b3c] mb-10">
        Recent Activity
      </h2>
        ...
        {activities.map((item, index) => (
          <div key={item.id} className="relative flex gap-6">
            ...
            {index !== activities.length - 1 && (
              <div className="absolute left-5 top-10 w-[2px] h-10 bg-slate-100" />
            )}
            ...
            <div className="flex flex-col justify-center">
              <p className="text-[15px] text-slate-600">
                ...
                {item.action}{" "}
                <span className="font-bold text-slate-800">{item.target}</span>
              </p>
